...
impl GameSession {
    ...
    pub fn read_winner(&self) -> Option<&(String, u32)> {
        let majority = (self.nb_games / 2) + 1;

        if self.p1.1 >= majority && self.p1.1 > self.p2.1 {
            return Some(&self.p1);
        }
        if self.p2.1 >= majority && self.p2.1 > self.p1.1 {
            return Some(&self.p2);
        }
    ...
    pub fn update_score(&mut self, user_name: &str) {
        ...
        if self.p1.1 >= majority || self.p2.1 >= majority {
            return;
        }
